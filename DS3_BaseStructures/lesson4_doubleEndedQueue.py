'''
    ...
    双端队列(deque)：一系列元素的有序结合。其两端成为队首（front）和队尾（rear），元素在到达两端之前，始终位于双端队列中
    与队列不同的地方在于：双端队列对元素的添加和删除限制不那么严格，元素可以从两端插入也可以从两端删除。
    ...
    抽象数据类型（ADT）
        ...
        removeFront()   在队首插入一个元素，无参数，返回该移除的元素，双端队列会被改变
        removeRear()    在队尾插入一个元素，无参数，返回该移除的元素，双端队列会被改变
        isEmpty()       判断双端队列是否为空，无参数，返回布尔值
        size()          返回双端队列中的数据项的个数，无参数，返回值为整数数值
    Python3实现ADT：
...
'''
    lettcode  简单第9题  判断一个整数是否是回文数（回文词）
    回文数：123321
    回文词：上海来自本来自海上   山西运煤车煤运西山  roor   madam
